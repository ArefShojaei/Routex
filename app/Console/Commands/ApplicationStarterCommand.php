...
final class ApplicationStarterCommand extends Command {
    ...
    private function getHost(array $params): string {
        return array_key_exists("host", $params) ? $params["host"] : self::HTTP_DEFAULT_APPLICATION_HOST;
    }

    private function getPort(array $params): int {
        return array_key_exists("port", $params) ? $params["port"] : self::HTTP_DEFAULT_APPLICATION_PORT;
    }
}